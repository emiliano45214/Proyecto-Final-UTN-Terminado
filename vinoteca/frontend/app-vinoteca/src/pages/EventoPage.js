...
const EventoPage = (propt) => {
  ...
  const handleSubmit = async e => {
    ...
    setMsg('');
    setSending(true)
    const response = await axios.post('http://localhost:3000/api/eventos', formData);
    setSending(false);
  ...
  return (
    <main className="main">
      <div id="form-main">
        <div id="form-div">
          <h1>¡Contactanos!</h1>
          <p>envianos tu informacion a travez de este formulario junto a una breve descripcion de tu evento.</p>
          <form className="form" id="form1" action="/eventos" method="post" onSubmit={handleSubmit}>
            ...
            <p className="text">
              <textarea name="mensage" className="validate[required,length[6,300]] feedback-input" id="comment" placeholder="descripcion del evento" value={formData.mensage} onChange={handleChange}></textarea>
            </p>
